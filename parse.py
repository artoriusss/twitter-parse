...
async def worker(semaphore: asyncio.Semaphore, api: twscrape.API, q: str, keywords: bool = True):
    TwitterWrapper._create_dirs(TwitterWrapper())
    tweets = []
    async with semaphore:  
        try:
            if keywords:
                async for doc in api.search(q): # for search by keyword
